...
# Create VPC
VPC_ID=$(aws ec2 create-vpc --cidr-block "10.208.0.0/16" --instance-tenancy "default" \
  --tag-specifications "ResourceType=vpc,Tags=[{Key=Name,Value=${VPC_NAME}-vpc}]" \
  --query 'Vpc.VpcId' --output text)
aws ec2 modify-vpc-attribute --vpc-id $VPC_ID --enable-dns-hostnames

# Public Subnet 1
SUBNET_PUBLIC1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.208.1.0/24" \
...
# Public Subnet 2
SUBNET_PUBLIC2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.208.2.0/24" \
...
# Private Subnet 1
SUBNET_PRIVATE1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.208.3.0/24" \
  --availability-zone $AVAILABILITY_ZONE1 \
  --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=${VPC_NAME}-subnet-private1-${AVAILABILITY_ZONE1}}]" \
  --query 'Subnet.SubnetId' --output text)

# Private Subnet 2
SUBNET_PRIVATE2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.208.4.0/24" \
  --availability-zone $AVAILABILITY_ZONE2 \
